...
from config_data.config import type_top, type_meny_check
from database.core import User
from keyboards.inline.inline_button import markup_line_top, markup_line_next
...
@bot.message_handler(func=lambda message: message.text in type_meny_check[4])
def send_top_250(message: Message) -> None:
    ...
    markup = markup_line_top()
    mess = 'Выберите Топ фильмов'
    bot.send_message(message.chat.id, mess, reply_markup=markup)  # ответ на сообщение
    bot.set_state(message.from_user.id, UserState.top, message.chat.id)  # изменение статуса


# сюда получаем сообщение из markup_line
@bot.callback_query_handler(func=lambda call: call.data.isdigit() is True, state=UserState.top)
def callback_query(call: Any) -> None:
    ...
    """
    if call.data in type_top:  # проверяем ответы
        sort_type_data = '-1'  # сортировка рейтинга от 10 до 0
        type_sort_data = type_top[call.data]  # тип выбранного фильма
        data_string_request = string_request(sort_type_data=sort_type_data, type_sort_data=type_sort_data)
        ...
        with bot.retrieve_data(call.message.chat.id) as data:  # создаем временную переменную для хранения данных
            data["type_sort_data"] = type_sort_data
            data["page_data"] = 2
        ...
        bot.set_state(call.message.from_user.id, UserState.top, call.message.chat.id)  # изменение статуса


@bot.callback_query_handler(func=lambda call: call.data == 'next', state=UserState.top)
def callback_query_next(call: Any) -> None:
    ...
    with bot.retrieve_data(call.message.chat.id) as data:  # создаем временную переменную для хранения данных
        if call.data == 'next':  # проверяем ответы
            ...
            sort_type_data = '-1'  # сортировка рейтинга от 10 до 0
            type_sort_data = data["type_sort_data"]  # тип выбранного фильма
